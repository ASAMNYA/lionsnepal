<?php
    ...
    class RegisterController extends Controller
    {
        ...
        public function showRegistrationFormEmployer()
        {

            return view('frontend.auth.register_employer');
        }
        ...
        public function registerEmployer(RegisterRequest $request)
        {
            ...
            if (config('access.users.confirm_email')) {
                $user = $this->user->createEmployer($request->only('first_name' , 'last_name' , 'email' , 'password'));
                event(new UserRegistered($user));

                return redirect($this->redirectPath())->withFlashSuccess(trans('exceptions.frontend.auth.confirmation.created_confirm'));
            } else {
                access()->login($this->user->createEmployer($request->only('first_name' , 'last_name' , 'email' , 'password')));
                event(new UserRegistered(access()->user()));
